...
resource "datadog_monitor" "my-fastapi-app" {
  ...
  query = "max(last_1m):sum:kubernetes.containers.running{short_image:my-fastapi-app} <= 1"
  ...
  tags = ["app:my-fastapi-app", "env:demo"]
}